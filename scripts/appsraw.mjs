...
tv.on("connect", async () => {
  ...
  console.log("count:", r.launchPoints.length);
  console.log("FIELDS:", Object.keys(r.launchPoints[0]).join(", "));
  console.log("\nSAMPLE:", JSON.stringify(r.launchPoints[0], null, 2));
  console.log("\nICONS:");
  for (const p of r.launchPoints.slice(0, 6)) console.log(" ", p.id, "|", p.icon);
  const inputs = await tv.request("ssap://tv/getExternalInputList");
  console.log("\nINPUTS:", JSON.stringify(inputs.devices, null, 2).slice(0, 1200));
